...
async function verifyTeacherCodeForPhase(phaseId, inputEl, msgEl, callback) {
  const value = inputEl.value.trim();
  if (!value) {
    msgEl.textContent = 'Introduce el código de verificación docente.';
    msgEl.className = 'status-text empty';
    return;
  }
  const hash = await sha256Hex(value);
  if (hash === TEACHER_CODE_HASH) {
    state.unlockedTests[phaseId] = true;
    saveState();
    inputEl.value = '';
    msgEl.textContent = 'Verificación correcta. Test desbloqueado.';
    msgEl.className = 'status-text saved';
    callback();
  } else {
...
function renderTestCard(phase) {
  ...
  const unlocked = !!state.unlockedTests?.[testId];
  ...
  wrapper.innerHTML = `
    ...
    <div class="verification-panel">
      <label for="verify_${testId}"><strong>Verificación docente</strong></label>
      <div class="inline-actions">
        <input id="verify_${testId}" class="verify-input" type="password" autocomplete="off" placeholder="${unlocked ? 'Test desbloqueado' : 'Introduce el código'}" ${unlocked ? 'disabled' : ''}>
        <button class="primary-btn unlock-test-btn" ${unlocked ? 'disabled' : ''}>Verificar</button>
        <span class="status-text ${unlocked ? 'saved' : 'empty'}">${unlocked ? 'Test desbloqueado para esta fase.' : 'La práctica se entrega en EVAGD y, después, se valida aquí el acceso al test.'}</span>
      </div>
    </div>
    <div class="inline-actions" style="margin-top:14px;">
      <button class="primary-btn start-test-btn" ${unlocked ? '' : 'disabled'}>${testState.attemptsUsed >= 2 ? 'Ver intento final' : 'Iniciar / continuar test'}</button>
  ...
  const verifyBtn = wrapper.querySelector('.unlock-test-btn');
  const verifyInput = wrapper.querySelector('.verify-input');
  const verifyMsg = wrapper.querySelector('.verification-panel .status-text');

  const afterUnlock = () => {
    verifyInput.placeholder = 'Test desbloqueado';
  ...
  if (!unlocked) {
    verifyBtn.addEventListener('click', () => verifyTeacherCodeForPhase(testId, verifyInput, verifyMsg, afterUnlock));
    verifyInput.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        verifyTeacherCodeForPhase(testId, verifyInput, verifyMsg, afterUnlock);
      }
    });
  } else {
    showQuiz(testId, phase.test.pool, quizContainer, wrapper);
  ...
  resetBtn.addEventListener('click', () => {
    ...
    state.unlockedTests[testId] = false;
    saveState();
    renderPhases();
  });
...
function updateCertificate() {
  ...
  sheet.innerHTML = `
    <div class="certificate-title">
      ...
      <p>Acceso evaluable condicionado a verificación docente registrada.</p>
